...
def get_graph(fsm, title=None, missing=None):
    ...
    for state in [fsm.init_state] + fsm.states:
        ...
        if state.name == 'Start' or state.name == 'End':
            fsm_graph.add_node(n=state.name, shape=shape, color=color, style='filled')
        elif "queries" in state.name:
            fsm_graph.add_node(n=state.name, shape=shape, color='white', 
                style='rounded', fontsize = 24)
            frac = fsm_graph.subgraph(nbunch=[state.name], style='solid')
        else:
            fsm_graph.add_node(n=state.name, shape=shape, color=color)
    ...
    for src, input_value, dst in fsm.all_transitions:
        weight = float(dict(src.output_values)[input_value])
        if weight < 0.01:
            fsm_graph.add_edge(src.name, dst.name, label='Negligible', 
                fontsize=15, fontname='Bitstream Vera Sans', style='dotted')
        else: 
            freq = '{0:.4}'.format(weight)
            fsm_graph.add_edge(src.name, dst.name, label=freq, 
                fontsize=15, fontname='Bitstream Vera Sans', 
                style="setlinewidth("+str(float(weight)*5)+")")
    if missing:
        for state in missing:
            fsm_graph.remove_node(state)
    return fsm_graph
